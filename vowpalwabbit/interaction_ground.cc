...
{
  double total_importance_weighted_reward;//the accumulated importance weighted reward of a policy which optimizes the given value
...
base_learner* interaction_ground_setup(VW::setup_base_i& stack_builder)
{
  ...
  new_options
    .add(make_option("igl", igl_option)
         .keep()
         .necessary()
